class Solution {
public:
    int dfs(int r, int c, vector<vector<int>>& matrix, int prevVal, vector<pair<int,int>>& directions){
        ...
        for(auto dir : directions){
            int nr = r + dir.first;
            int nc = c + dir.second;
            res = max(res, 1 + dfs(nr, nc, matrix, matrix[r][c], directions));
        }
    ...
    int longestIncreasingPath(vector<vector<int>>& matrix) {
        ...
        for(int r = 0; r < rows; r++){
            for(int c = 0; c < cols; c++){
                int temp = dfs(r, c, matrix, INT_MIN, directions);
                res = max(res, temp);
